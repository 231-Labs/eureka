...
/// Test decryption for OLD FORMAT Sculpts (without printer_id parameter)
/// 
/// This example is for Sculpts encrypted with the old seal_approve signature
/// that only had _id parameter (no printer_id)
/// 
/// Usage: cargo run --example test_old_format -- <sculpt_id>
/// Example: cargo run --example test_old_format -- 0x90b0f833191657d31b386f4c1f7c4aa2f39622956e2d5fae820cc4dda4b0044e
/// 
/// NOTE: Use test_new_contract.rs for newly minted Sculpts
...
/// Decrypt sculpt using Seal SDK (OLD FORMAT - no printer_id)
async fn decrypt_sculpt(
    ...
    // Prepare arguments for seal_approve (OLD FORMAT - only id parameter)
    let id_arg = builder.pure(id_bytes)?;

    // Call seal_approve function in the sculpt module
    // Parameters: _id: vector<u8> only (OLD FORMAT)
